...
from tests.unit.fixtures.auth_panel import AdminTestUOW
from tests.unit.fixtures.user import ProfileTestUOW

pytest_plugins = (
    "tests.unit.fixtures.auth_panel",
    "tests.unit.fixtures.user",
)

@pytest.fixture(scope="session")
def alembic_config() -> Config:
    """Initialize the Alembic Config for pytest_alembic."""
    ini_path = os.path.join(os.path.dirname(__file__), "src/apps/alembic.ini")
    return Config(file_=ini_path)
...
async def app_fixture(
    ...
    def generator_dependency_override() -> Generator[tuple[Any, Any], None, None]:
        ...
        dict_dep_overrides = {
            ...
            AuthUOW: AdminTestUOW,
            UserUOW: ProfileTestUOW,
        }
